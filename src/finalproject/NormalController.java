 ...
 @FXML
    ...
    void text(MouseEvent event) {
        ...
        if(Vitamin.isPressed()){
         masseg.setText("Vitamin C is found in food and supplements, and has proven skin care benefits when applied topically to the skin.\n\n"
                 + "How to apply?\n" +
             ...
             masseg.setText("How to apply?\n"+
                            "1.Hydration supports the skin's natural lipid barrier, helping it to better retain water and repel external irritants \n" +
                            "2.Warm up moisturizer in hands. \n" +
                            "3.Apply to cheeks using gentle circular motions. \n" +
                            "4.Apply to forehead and rest of face.");
            }
...
@FXML
    ...
    void goList(ActionEvent event) throws IOException {
        root=FXMLLoader.load(getClass().getResource("routineN.fxml"));
        stage=(Stage) ((Node)event.getSource()).getScene().getWindow();
        scene = new Scene(root);
        stage.setScene(scene);
        stage.show();
